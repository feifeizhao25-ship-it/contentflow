...
export class FalImageGenerationService {
    ...
    constructor() {
        // 后端 AIController 是 @Controller('ai') + @Post('generate/image')，
        // 配合 setGlobalPrefix('api/v1') 实际路径是 /api/v1/ai/generate/image。
        // 此前这里写 '/api/ai'，少了 v1 —— Next 的 /api/v1/[...path] 代理
        // 匹配不到，请求一直 404。
        this.apiBaseUrl = '/api/v1/ai';
    ...
    async generateImage(params: FalImageGenerationParams): Promise<FalImageGenerationResult> {
        try {
            ...
            // 后端全局 TransformInterceptor 把响应包成 { success, data, meta }，
            // 直接读 envelope.url 永远是 undefined
