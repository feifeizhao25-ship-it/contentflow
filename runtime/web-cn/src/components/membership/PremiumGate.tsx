...
export const useSubscription = () => {
    ...
    const refresh = useCallback(async () => {
        try {
            // NestJS 全局挂了 TransformInterceptor，所有响应都被包成
            // { success, data, meta }。此前直接读 data.plan 永远是 undefined，
            // 于是所有付费用户都被当成 free —— 付费墙「宁可错杀」的兜底
            // 恰好掩盖了这个 bug，表现成「买了也用不了」。
            const envelope = await apiClient.get<{ data?: Partial<UserSubscription> }>(
