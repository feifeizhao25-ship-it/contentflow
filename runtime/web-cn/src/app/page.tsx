...
export default function Home() {
  ...
  const plans = [
    { name: '免费版', price: '¥0', note: '适合验证工作流', features: ['基础内容创建', '有限任务额度', '人工发布检查'] },
    { name: '专业版', price: '¥99/月', note: '适合个人创作者', features: ['更多 AI 创作额度', '内容日历与复盘', '素材与人设管理'], recommended: true },
    { name: '团队版', price: '¥299/月', note: '适合小型运营团队', features: ['成员与角色协作', '审核发布流程', '团队数据看板'] },
    { name: '企业版', price: '¥899/月', note: '适合多品牌组织', features: ['企业权限与审计', '开放接口与集成', '专属实施支持'] },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fc] text-slate-950">
      ...
      <main>
        <section className="relative overflow-hidden px-4 pb-20 pt-16 sm:pt-24"><div className="absolute inset-0 bg-[radial-gradient(circle_at_78%_18%,rgba(99,102,241,.18),transparent_32%),radial-gradient(circle_at_18%_70%,rgba(168,85,247,.12),transparent_32%)]" /><div className="relative mx-auto grid max-w-6xl items-center gap-12 lg:grid-cols-[1.08fr_.92fr]"><div><p className="inline-flex rounded-full border border-indigo-200 bg-indigo-50 px-4 py-2 text-sm font-bold text-indigo-700">面向中国内容团队的 AI 发布工作台</p><h1 className="mt-6 text-4xl font-black leading-tight sm:text-6xl">内容只做一次，<br /><span className="text-indigo-600">每个平台都认真对待。</span></h1><p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">从选题、创作、平台适配到审核、排期和复盘，把重复劳动交给系统，把品牌判断留给人。</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link href="/register" className="rounded-xl bg-indigo-600 px-6 py-3 text-center font-bold text-white shadow-lg shadow-indigo-200 hover:bg-indigo-500">免费建立第一个工作区</Link><Link href="/login" className="rounded-xl border border-slate-300 bg-white px-6 py-3 text-center font-bold text-slate-700 hover:border-indigo-300">已有账号，直接登录</Link></div><p className="mt-4 text-sm text-slate-500">发布前人工确认 · 无真实数据不做确定归因 · 不承诺流量或收益结果</p></div><div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-2xl shadow-indigo-100"><div className="flex items-center justify-between border-b border-slate-100 pb-4"><div><p className="text-sm text-slate-500">今日工作台</p><p className="mt-1 text-xl font-black">先处理最重要的一件事</p></div><span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-bold text-amber-700">待人工确认</span></div><div className="mt-5 space-y-3">{['检查平台规则与敏感表达', '确认引用来源和采集日期', '逐平台预览标题、封面与正文', '批准后进入排期队列'].map((item, index) => <div key={item} className="flex items-center gap-3 rounded-xl bg-slate-50 p-3"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-xs font-black text-indigo-700">{index + 1}</span><span className="text-sm font-medium text-slate-700">{item}</span></div>)}</div></div></div></section>
        ...
        <section id="plans" className="bg-white px-4 py-20"><div className="mx-auto max-w-6xl"><div className="max-w-2xl"><p className="text-sm font-black tracking-widest text-indigo-600">会员方案</p><h2 className="mt-3 text-3xl font-black sm:text-4xl">从个人创作到企业协作</h2><p className="mt-4 text-slate-600">具体额度和权益以下单页及服务端校验为准，升级不会绕过支付确认。</p></div><div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-4">{plans.map((plan) => <article key={plan.name} className={`relative rounded-2xl border p-6 ${plan.recommended ? 'border-indigo-500 shadow-xl shadow-indigo-100' : 'border-slate-200'}`}>{plan.recommended && <span className="absolute -top-3 left-5 rounded-full bg-indigo-600 px-3 py-1 text-xs font-bold text-white">个人创作者常用</span>}<h3 className="text-xl font-black">{plan.name}</h3><p className="mt-3 text-3xl font-black">{plan.price}</p><p className="mt-2 text-sm text-slate-500">{plan.note}</p><ul className="mt-5 space-y-3 text-sm text-slate-700">{plan.features.map((feature) => <li key={feature}>✓ {feature}</li>)}</ul><Link href="/register" className={`mt-6 block rounded-xl px-4 py-2.5 text-center text-sm font-bold ${plan.recommended ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-800'}`}>选择{plan.name}</Link></article>)}</div></div></section>
