...
export class PublishProcessor {
  ...
  async dispatch(job: Job<PublishJobData>) {
    ...
    if (!webhookUrl || !webhookSecret) {
      throw new Error('Production publishing dispatcher is not configured');
    }
    if (webhookSecret.length < 32) {
      throw new Error('Publishing dispatcher secret must contain at least 32 characters');
    }
    const parsedWebhook = new URL(webhookUrl);
    if (parsedWebhook.protocol !== 'https:') {
      throw new Error('Publishing dispatcher must use HTTPS');
    }
    ...
    await this.prisma.publishTask.update({
      where: { id: task.id },
      data: { status: 'processing', started_at: new Date(), error_message: null },
    });

    const payload = JSON.stringify({
      schemaVersion: 1,
      idempotencyKey: `contentflow:${task.id}`,
      taskId: task.id,
      tenantId: task.tenant_id,
      marketRegion: this.config.get<string>('MARKET_REGION'),
      ...
      content: {
        ...
        bodyHtml: task.content.body_html,
        coverUrl: task.content.cover_url,
        mediaUrls: task.content.media_urls,
    ...
    try {
      const response = await fetch(webhookUrl, {
        ...
        headers: {
          ...
          'X-ContentFlow-Signature': `sha256=${signature}`,
        },
        body: payload,
      ...
      if (published && !result.remotePostId) {
        throw new Error('Dispatcher claimed publication without a remote post ID');
      }
      return this.prisma.publishTask.update({
        where: { id: task.id },
        data: {
          status: published ? 'published' : 'submitted_unconfirmed',
          platform_post_id: result.remotePostId ?? null,
          platform_post_url: result.remotePostUrl ?? null,
          completed_at: published ? new Date() : null,
          error_details: { remote_state: state },
        },
      });
    } catch (error) {
