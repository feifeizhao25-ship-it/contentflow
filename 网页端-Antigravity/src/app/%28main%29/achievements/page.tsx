...
export default function AchievementsPage() {
    ...
    const { balance, addPoints, getLevelProgress } = usePointsStore();
    const { currentXP: experiencePoints } = getLevelProgress();
    ...
    const handleClaimReward = (achievement: typeof ACHIEVEMENTS[0]) => {
        if (!unlockedIds.includes(achievement.id)) {
            ...
            addPoints(achievement.reward, 'achievement_reward', `成就奖励: ${achievement.name}`);
        }
    };
