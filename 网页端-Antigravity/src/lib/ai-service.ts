...
export class SiliconFlowService {
    ...
    constructor() {
        // 仅从服务端环境变量读取。
        // 不再使用 NEXT_PUBLIC_ 前缀——那会把密钥打进浏览器 bundle；
        // 也不再提供硬编码 fallback——那会让密钥随源码一起泄露。
        this.apiKey = process.env.SILICONFLOW_API_KEY || '';
        this.baseURL = 'https://api.siliconflow.cn/v1';
        if (!this.apiKey) {
            throw new Error(
                'SILICONFLOW_API_KEY is not configured. Set it as a server-side environment variable.'
            );
        }
    }
...
export class AIService {
    private service: OpenRouterService | SiliconFlowService;

    constructor(provider?: 'openrouter' | 'siliconflow') {
        const selectedProvider = provider || (process.env.NEXT_PUBLIC_AI_PROVIDER as any) || 'siliconflow';
     ...
     * 调用后端 API 生成文章 (推荐)
     */
    async generateArticleBackend(params: {
        topic: string;
        style?: string;
        platform?: string;
        keywords?: string[];
        ...
        const res = await apiClient.post<any>('/ai/generate/article', params);
        return res.data;
    }
     ...
     * 调用后端 API 生成标题
     */
    async generateTitlesBackend(params: {
        topic: string;
        platform?: string;
        count?: number;
    }): Promise<string[]> {
        const res = await apiClient.post<any>('/ai/generate/titles', params);
        return res.data.titles;
    }

    /**
     * 本地生成文章 (Legacy/Fallback)
     */
    async generateArticle(params: AIGenerationParams & { personaId?: string }): Promise<AIGenerationResult> {
        // 先尝试后端
        try {
            const res = await this.generateArticleBackend({
                topic: params.prompt,
                style: params.style,
            ...
            console.warn('Backend AI failed, falling back to direct API', e);
    ...
    async generateTitles(topic: string, count?: number): Promise<string[]> {
        try {
            return await this.generateTitlesBackend({ topic, count });
        } catch (e) {
            return this.service.generateTitles(topic, count);
        }
    }

    async rewriteContent(content: string, style?: string): Promise<AIGenerationResult> {
        const prompt = `请改写以下内容，保持核心意思不变，但使用不同的表达方式：\n\n${content}`;
        return this.service.generateText({ prompt, style: style as any });
    }
}
...
export class FalImageGenerationService {
    ...
    async generateImage(params: FalImageGenerationParams): Promise<FalImageGenerationResult> {
        try {
            console.log('Generating image via backend (fal.ai)...');
            const res = await apiClient.post<any>('/ai/generate/image', params);
            return res.data;
        } catch (error) {
            console.warn('Backend image generation failed, falling back to local /api/ai/generate/image', error);
